...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      _this.$http
          .post("http://localhost:3000/wangkachongzhi/ka", wangForm)
          .then(function(response) {
            ...
            if (response.data.code == "200") {
              ...
              _this.resultForm.balance = Data.yueka,
              _this.resultForm.lastTime = Data.timeka.substring(0,10),
              _this.resultForm.laseMoney = Data.lastka
    ...
    recharge() {
      ...
      _that.$http
          .post("http://localhost:3000/wangkachongzhi/updateka", M)
          .then(function(response) {
            console.log(response.data);
            // 如果状态码为200，代表登录成功，0为失败
            if (response.data.code == "200") {
              ...
              _that.resultForm.balance = Data.yueka,
              _that.resultForm.lastTime = Data.timeka.substring(0,10),
              _that.resultForm.laseMoney = Data.lastka
              // 如果存在该用户,弹出登录成功提示信息
              _that.$message({
                showClose: true,
                message: "充值成功",
