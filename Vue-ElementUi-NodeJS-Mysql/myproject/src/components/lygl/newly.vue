...
export default {
  ...
  methods: {
    ...
    onChange(file) {
      // var _this = this;
      // console.log(fileList);//上传成功会输出上传文件
      // console.log(file,"这里是file")
    ...
    readExcel(e) {
      ...
      console.log(files,"这里是readExcel中的files");
      if (!files) {
        //如果没有文件
        return false;
      ...
      fileReader.onload = ev => {
        ...
        try {
          ...
          // console.log(workbook.SheetNames,"这里是workbook.SheetNames");
          ...
          // console.log(that.addexl,"这里是循环后的addexl")//addexl是新添加的表格中的数据
          that.$http
        .post("http://localhost:3000/newly/addexl", {
          "addexl":that.addexl
        })
        .then(function(response) {
          console.log(response.data.code,"这里是data.code");
          console.log(that)
          if (response.data.code == 200) {
           that.$http
                .post("http://localhost:3000/newly", that.queryInfo)
                .then(function(response) {
                  console.log(response.data);
                  if (response.data.code == 200) {
                    that.lylist = response.data.data;
                    that.total = response.data.total;
                  }
                });
          }
        });
          // this.$refs.upload.value = that.addexl;
        } catch (e) {
          return false;
        }
      };
  ...
  created() {
    ...
    this.$http
      ...
      .then(function(response) {
        ...
        if (response.data.code == 200) {
          _this.lylist = response.data.data;
          console.log(response.data.data,'ekighilguhsiughliughle')
          _this.total = response.data.total;
        }
      });
    // 获取楼宇下拉选择框的全部楼宇的名字
    this.$http
      ...
      .then(function(response) {
        ...
        if (response.data.code == 200) {
          _this.lyNamelist = response.data.data;
          // console.log(response.data.data,"ghalhalwieughalweigalieghalwieugh")
        }
      });
  }
};
