...
// prefix为路由前缀，所有路由都会加上这个前缀，否则请求不到

// 搜索按钮点击事件搜索相应内容并返回
router.post('/search', async (ctx, next) => {
	// console.log(ctx.request.body,'这里是宿舍列表search==================================================');
	...
	// 得到lyid（这里的意思是根据查询时输入框输入的内容，去lynumber表中查询例如西区一共与对应的号码为1，拿到1这个号码）
	let lyid = result[0].id;
	// 根据lyid 在宿舍表中查找对应楼宇的所有宿舍
	var b = '';
	if (ssnumber == '') {
		// 没有输入宿舍号的情况
		// 根据lyid （西区一公寓为号码1）在宿舍表中查找对应楼宇的所有宿舍
