...
router.post('/', async (ctx, next) => {
	...
	// }
	// 起始页索引（表示该页的数据从数据库中的第几条数据开始查询，例如一页有五条数据第二页的第一条数据，就是从第六条开始因为是开区间所以toppageIndex为5）
	let toppageIndex = (pagenum - 1) * pagesize;


	//查询数据库 这段语句的意思应该是从toppageIndex（起始页）开始取pagesize大小条数据
...
router.post('/addexl', async (ctx, next) => {
	...
	for(let i=0;i<ctx.request.body.addexl.length;i++){
		...
		let leader = ctx.request.body.addexl[i].leader;
		let sex = ctx.request.body.addexl[i].sex;
		let tel = ctx.request.body.addexl[i].tel;
		// 将信息添加到数据库
		let a = await query(`insert into lynumber(lynumber,leader,sex,tel,state) values('${lynumber}','${leader}','${sex}','${tel}',1)`);
			if (a.affectedRows == 1) {
				// 代表插入成功
				var flag=1;
			}else{
