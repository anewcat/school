...
router.post('/add', async (ctx, next) => {
	...
	// 将信息添加到数据库
	let a = await query(
		`insert into user(pass,state,uname,name,sex,tel,sushe,xueyuan,banji,lynumber,fdy,fdytel,family,familytel,roler) values('${pass}','${state}','${uname}','${name}','${sex}','${tel}','${sushe}','${xueyuan}','${banji}','${lynumber}','${fdy}','${fdytel}','${family}','${familytel}','${roler}')`);
	console.log(a);
	if (a.affectedRows == 1) {
		// 代表插入成功
		let data = {
...
router.post('/addexl', async (ctx, next) => {
	for(let i=0;i<ctx.request.body.addexl.length;i++){
		// let id = ctx.request.body.addexl[i].id;
		let state='1'
		let pass="12345666"
		let uname = ctx.request.body.addexl[i].uname;
		let name = ctx.request.body.addexl[i].name;
		let sex = ctx.request.body.addexl[i].sex;
		let tel = ctx.request.body.addexl[i].tel;
		let xueyuan = ctx.request.body.addexl[i].xueyuan;
		let banji = ctx.request.body.addexl[i].banji;
		let lynumber = ctx.request.body.addexl[i].lynumber;
		let sushe = ctx.request.body.addexl[i].sushe;
		let roler = 0
		// 将信息添加到数据库
		let a = await query(`insert into user(pass,state,uname,name,sex,tel,sushe,xueyuan,banji,lynumber,roler) values('${pass}','${state}','${uname}','${name}','${sex}','${tel}','${sushe}','${xueyuan}','${banji}','${lynumber}','${roler}')`);
			if (a.affectedRows == 1) {
				// 代表插入成功
				var flag=1;
			}else{
